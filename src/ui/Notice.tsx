...
const TONES: Record<Tone, { icon: IconName; box: string; role: 'status' | 'alert' }> = {
  info: {
    icon: 'info',
    box: 'border-solid border-rule bg-paper text-ink',
    role: 'status',
  },
  warning: {
    icon: 'clock',
    box: 'border-dashed border-state-pending bg-state-pending-bg text-ink',
    role: 'status',
  },
  danger: {
    icon: 'alert',
    box: 'border-solid border-state-failed bg-state-failed-bg text-ink',
    role: 'alert',
  },
  ambiguous: {
    icon: 'question',
    box: 'border-dotted border-2 border-ink bg-paper text-ink',
    role: 'alert',
  },
}
...
export function Notice({ tone, title, body, action, supportRef }: NoticeProps) {
  const t = TONES[tone]
  return (
    <div role={t.role} className={`flex gap-3 rounded-slab border p-4 ${t.box}`}>
